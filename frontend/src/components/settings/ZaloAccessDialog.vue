<template>
  <v-dialog v-model="open" max-width="520">
    <v-card>
      <v-card-title class="d-flex align-center pr-10">
        <v-icon class="mr-2" color="cyan">mdi-shield-account</v-icon>
        <span class="text-truncate">Phân quyền truy cập — {{ accountName }}</span>
      </v-card-title>
        ...
        <div v-if="accessList.length" class="mb-4">
          ...
          <v-list density="compact" rounded="lg" variant="tonal">
            <v-list-item v-for="a in accessList" :key="a.id">
              <template #prepend>
                <v-avatar color="cyan-lighten-4" size="32" class="mr-3">
                  <v-icon color="cyan" size="20">mdi-account</v-icon>
                </v-avatar>
              </template>
              <v-list-item-title class="font-weight-bold">{{ a.user?.fullName || 'Nhân viên' }}</v-list-item-title>
              <v-list-item-subtitle>{{ a.user?.email }}</v-list-item-subtitle>
              <template #append>
                <v-select
...
async function handleUpdatePermission(accessId: string, permission: string) {
  try {
    await api.post(`/zalo-accounts/${props.accountId}/access/${accessId}`, { permission });
    await fetchAccess();
  } catch (err: any) {
