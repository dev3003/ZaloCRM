  ...
  <div class="conversation-list d-flex flex-column" style="width: 100%; border-right: 1px solid var(--border-glow, rgba(0,242,255,0.1)); height: 100%;">
    ...
    <div class="pa-2">
      ...
      <div class="d-flex align-center ga-1">
        <v-text-field
          ...
          class="flex-grow-1"
        />
        <ZaloFriendSearchDialog :initial-account-id="selectedAccountId" />
      </div>
      ...
      >
        <v-tab value="all" class="text-none">Tất cả</v-tab>
        <v-tab value="unread" class="text-none">
          Chưa đọc
          <v-badge
            v-if="unreadCount > 0"
            :content="unreadCount"
    ...
    <v-list class="flex-grow-1 overflow-y-auto pa-0" density="compact">
      ...
      <v-list-item
        ...
        :class="{ 'active': conv.id === selectedId, 'unread': conv.unreadCount > 0 }"
      >
        <template #prepend>
...
import ZaloFriendSearchDialog from './ZaloFriendSearchDialog.vue';
